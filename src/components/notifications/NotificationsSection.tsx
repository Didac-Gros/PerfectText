...
import { Phone, MessageCircle, Heart, X, Check, Clock } from 'lucide-react';
    ...
    <div className="flex-1 p-8 max-w-4xl mx-auto">
      ...
      <header className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-3xl font-semibold text-gray-900 mb-3 tracking-tight">Notificaciones</h1>
        </div>
          ...
          >
            ...
            {unreadCount > 0 && (
              <span className={`px-2 py-0.5 text-xs rounded-full ${
                filter === 'unread' ? 'bg-white text-gray-900' : 'bg-red-100 text-red-600'
              }`}>
                {unreadCount}
              </span>
            )}
                  ...
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <p className="text-sm text-gray-900">
                        ...
                        <span className="text-gray-600 ml-1">{getNotificationText(notification)}</span>
                      </p>
                      <p className="text-xs text-gray-500 mt-1">
                        {notification.user.year} • {notification.user.major} • {notification.timestamp}
                      </p>
                    </div>
                    ...
                    <div className="mt-3 p-3 bg-gray-50/80 rounded-xl">
                      ...
                      {notification.feelContent && (
                        <p className="text-xs text-gray-500 italic">
                          En respuesta a: "{notification.feelContent.substring(0, 50)}..."
                        </p>
                      )}
                    </div>
                  )}

                  {notification.type === 'reaction' && notification.feelContent && (
                    <div className="mt-3 p-3 bg-gray-50/80 rounded-xl">
                      <p className="text-sm text-neutral-600 italic tracking-wide leading-relaxed">
                        En tu feel: "{notification.feelContent.substring(0, 60)}..."
                      </p>
                    </div>
                  )}
                </div>
